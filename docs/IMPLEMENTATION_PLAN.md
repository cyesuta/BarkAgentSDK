...
- [ ] 建立 `BarkAgentSDK/package.json`
  ...
  ```
- [ ] 建立 `.github/workflows/publish.yml`（push tag → npm publish to GitHub Packages）
- [ ] 建立 `.npmrc`（指向 GitHub Packages registry）
  ```
  @cyesuta:registry=https://npm.pkg.github.com
  ```
- [ ] 建立 `index.mjs` 空 re-export 骨架
- [ ] 建立 `index.cjs`（`module.exports = require('./index.mjs')` wrapper）
- [ ] 建立 `README.md`（安裝 + 快速開始範例）
- [ ] 初始化 git，設定 remote 到 GitHub
- [ ] 在 GitHub 建立 public repo（`BarkAgentSDK`）
...
**`core/config.mjs`**
- [ ] 複製 `BarkConfig` class（`config.mjs`）
- [ ] 確認沒有 Tauri / sidecar 引用（目前應該是乾淨的）
- [ ] 加上 JSDoc 每個欄位含義

**`core/vault.mjs`**
- [ ] 複製 `WorkspaceVault` class 和 `obtainVault` / `dropVault` helpers
- [ ] 去除任何 `process.stdin` / `process.stdout` 引用
- [ ] 確認 vault 只管狀態，不做 I/O

**`pricing/`**
- [ ] 複製 `pricing/table.mjs`（含 `pricingTable`、`computeCharge`、`exchangeRate`）
- [ ] 複製 `pricing/tracker.mjs`（`usageTracker`）
- [ ] 確認無外部依賴

**驗證**
- [ ] `node --check core/config.mjs` ✅
- [ ] `node --check core/vault.mjs` ✅
- [ ] `node --check pricing/table.mjs` ✅
...
- [ ] 複製 registry（`registerProvider`、`resolveProvider`、`PROVIDER_ENV_MAP`）
- [ ] 去除任何 BarkIDE 特定 import

**`providers/deepseek.mjs`**
- [ ] 複製（含 `<think>` tag 解析 state machine）
- [ ] 確認 `parseOpenAIStream` helper 也一起搬

**`providers/openai-compat.mjs`**
- [ ] 複製（涵蓋 Kimi、Qwen、GLM、Grok、Doubao、Mimo、OpenRouter、MiniMax、Ollama）

**`providers/gemini.mjs`**
- [ ] 複製（含 Gemini content format 轉換、vision 支援）

**`providers/codex.mjs`**
- [ ] 複製
- [ ] 文件標注：需要外部 `codex` binary，其他 provider 無此要求

**驗證**
- [ ] `node --check providers/registry.mjs` ✅
- [ ] `node --check providers/deepseek.mjs` ✅
- [ ] `node --check providers/openai-compat.mjs` ✅
- [ ] `node --check providers/gemini.mjs` ✅
...
**`tools/action.mjs`**
- [ ] 複製 `defineAction`、`buildActionHub`
- [ ] 確認介面文件完整

**`tools/cycle.mjs`**
- [ ] 複製 `actionCycle`（max 25 rounds logic）
- [ ] 確認 `AbortSignal` 中止路徑完整

**`tools/builtins.mjs`**
- [ ] 複製 Read / Glob / Grep / Write / Edit / MultiEdit / Bash / web_search / fetch_url
- [ ] **重要改動**：`registerBuiltinActions(workspace, options = {})` 的 `workspace` 保持必填
- [ ] 加 `options.allowed` 陣列支援（讓 caller 選擇只開放哪些工具）
  ```js
  registerBuiltinActions('/my/project', { allowed: ['Read', 'Glob', 'Grep'] })
  ```
- [ ] Write / Edit / MultiEdit / Bash 的 workspace 限制邏輯保留（安全邊界）

**`tools/scanner.mjs`**
- [ ] 複製 `scanLocalActions`、`scanDispatcherTasks`
- [ ] **改動**：工具目錄路徑從 `<cwd>/.barkide/tools/` 改為可設定
  ```js
  scanLocalActions(cwd, { toolsDir: '.barkide/tools' })   // 預設值維持兼容
  ```

**`tools/vision.mjs`**
- [ ] 複製 `hasNativeVision`、`outlinePicture`、`directPicturePass`、`pictureNote`

**驗證**
- [ ] `node --check tools/action.mjs` ✅
- [ ] `node --check tools/cycle.mjs` ✅
- [ ] `node --check tools/builtins.mjs` ✅
- [ ] `node --check tools/scanner.mjs` ✅
...
**`skills/scanner.mjs`**
- [ ] 複製 `capabilityScanner`、`injectCapabilities`
- [ ] **改動**：skill 搜尋目錄從硬碼改為可設定
  ```js
  // 預設搜尋順序（維持兼容）：
...
**`skills/trigger.mjs`**
- [ ] 複製 `TriggerRule`、`TriggerBoard`
- [ ] 無 BarkIDE 特定依賴，應為直接搬移

**驗證**
- [ ] `node --check skills/scanner.mjs` ✅
- [ ] `node --check skills/trigger.mjs` ✅
...
**`client/events.mjs`**
- [ ] 實作輕量 EventEmitter（或直接用 Node.js `EventEmitter`）
- [ ] 事件列表：`text`、`thinking`、`tool_use`、`tool_result`、`done`、`error`
...
- [ ] 實作 `BarkClient` class
  ...
  ```
- [ ] `registerTool` 驗證 spec 有 `name`、`description`、`inputSchema`、`handler`
- [ ] `builtinTools` 處理：
  - `true` → `registerBuiltinActions(cwd)` 全部
  - `false` → 不加
  - `string[]` → `registerBuiltinActions(cwd, { allowed: [...] })`
- [ ] API key 環境變數自動設定（維持現有 `apiKeyEnv` 邏輯）
...
**驗證**
- [ ] `node --check client/events.mjs` ✅
- [ ] `node --check client/Session.mjs` ✅
- [ ] `node --check client/BarkClient.mjs` ✅
...
- [ ] `examples/basic.mjs`：基本 send + onText streaming
  ```js
  ...
  ```
- [ ] `examples/multi-turn.mjs`：多輪對話，驗證 vault 累積正確
- [ ] `examples/custom-tool.mjs`：自訂工具被模型呼叫
- [ ] `examples/file-tools.mjs`：Read / Grep / Glob 對真實檔案
- [ ] `examples/vision.mjs`：含圖片的 send（non-native provider → describe_image）
- [ ] `examples/abort.mjs`：mid-stream abort，確認 TurnResult.ok = false

**單元測試**（可選，工作量較大）
- [ ] `tests/config.test.mjs`：BarkConfig immutability
- [ ] `tests/vault.test.mjs`：obtainVault / dropVault lifecycle
- [ ] `tests/pricing.test.mjs`：已知 token 數 → 已知 cost
- [ ] `tests/action.test.mjs`：defineAction + dispatch happy path + error path
